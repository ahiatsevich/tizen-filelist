...
static fs_manager *_fs_manager_shared_instance = NULL;

fs_manager *fs_manager_get_shared_manager(void)
{
	if (_fs_manager_shared_instance == NULL) {
		_fs_manager_shared_instance = fs_manager_create();
		RETVM_IF(!_fs_manager_shared_instance, false, "Failed to create file system manager");
	}

	return _fs_manager_shared_instance;
}
...
//	}
	...
	storage_get_available_space(STORAGE_TYPE_INTERNAL, &pNode_device->size_availbale);
