...
static char *icon_array[FILE_TYPE_MAX] = {
	[FILE_TYPE_STORAGE] = FM_ICON_STORAGE,
...
int model_utils_read_dir(const char *dir_path, Eina_List **dir_list, Eina_List **file_list)
{
	...
	while ((readdir_r(pDir, &ent_struct, &ent) == 0) && ent) {
		...
		if (pNode) {
			...
			pNode->full_path = NULL;
			pNode->size = 0;
			pNode->name = strdup(ent->d_name);
			pNode->full_path = common_util_strconcat(pNode->parent_path, "/", pNode->name, NULL);
			...
			if (ent->d_type == DT_DIR) {
				...
				if (!stat(pNode->full_path, &file_stats))
				{
					pNode->size = file_stats.st_size;
				}
			}
